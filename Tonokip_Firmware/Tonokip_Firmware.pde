  ...
  if(code_seen('M'))
  {
    ...
    switch( (int)code_value() ) 
    {
      case 104: // M104
        ...
        break;
      case 115: // M115 HEATBED
        if (code_seen('S')) target_2_raw = temp2analog(code_value());
        break;
      case 105: // M105
        Serial.print("T:");
...
inline void manage_heater()
{
  ...
  //heatbed
  current_2_raw = analogRead(TEMP_1_PIN);                  // If using thermistor, when the heater is colder than targer temp, we get a higher analog reading than target, 
  if(USE_THERMISTOR) current_2_raw = 1023 - current_2_raw;   // this switches it up so that the reading appears lower than target for the control logic.
  
  
  if(current_raw >= target_raw){ digitalWrite(HEATER_0_PIN,LOW);
  }else{ digitalWrite(HEATER_0_PIN,HIGH);}
  
  if(current_2_raw >= target_2_raw){digitalWrite(HEATER_1_PIN,LOW);
